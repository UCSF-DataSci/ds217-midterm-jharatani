...
for nb in q4_exploration.ipynb q5_missing_data.ipynb q6_transformation.ipynb q7_aggregation.ipynb; do
    echo "Running $nb..." >> reports/pipeline_log.txt
    if jupyter nbconvert --to notebook --execute "$nb" --output "reports/${nb%.ipynb}_out.ipynb" >> reports/pipeline_log.txt 2>&1; then
        echo "Finished $nb" >> reports/pipeline_log.txt
    else
        echo "Error running $nb — pipeline stopped." >> reports/pipeline_log.txt
        exit 1
    fi
done
